...
def download_midi(job_id: str) -> FileResponse:
    ...
    if job.midi_path is None or not job.midi_path.exists():
        raise JobResultNotReadyError("File MIDI chưa sẵn sàng hoặc không tồn tại.")
...
def download_json(job_id: str) -> FileResponse:
    ...
    if job is None:
        raise JobNotFoundError(_JOB_NOT_FOUND_MESSAGE)
    if job.json_path is None or not job.json_path.exists():
        raise JobResultNotReadyError("File JSON chưa sẵn sàng hoặc không tồn tại.")
