...
- **Merge nốt liền kề cùng cao độ**: sắp xếp theo start time; merge nốt A vào B khi **cả ba** điều kiện sau đều đúng:
  ...
  - **`min(durA, durB) / max(durA, durB) < MERGE_MAX_DURATION_RATIO`** với `MERGE_MAX_DURATION_RATIO = 0.3` — chỉ gộp khi một nốt ngắn bất thường so với nốt kia.
...
**Bug nghiêm trọng phát hiện qua phản hồi người dùng + đã sửa (2026-07-17)**: điều kiện thứ 3 (duration ratio) **không có trong thiết kế ban đầu** — chỉ có 2 điều kiện đầu. Hậu quả: các nốt **lặp lại thật sự** cùng cao độ, đứng sát nhau (gap≈0, đặc biệt phổ biến với piano chơi tremolo/nhịp lặp) bị gộp nhầm thành MỘT nốt dài duy nhất.

Đã xác minh nguyên nhân bằng cách đọc trực tiếp `model_output['onset']` (onset activation matrix) từ Basic Pitch tại các điểm nối giữa các "mảnh" nốt bị gộp nhầm: giá trị onset **0.75–0.95** (rất cao, vượt xa `onset_thresh=0.5` mặc định của Basic Pitch) — xác nhận model đã rất tự tin đây là các lần đánh nốt (onset) thật sự riêng biệt, không phải nhiễu phân mảnh. Vì Basic Pitch **chỉ tạo note event mới khi onset activation vượt ngưỡng**, hai note event cùng cao độ đứng sát nhau (dù gap=0) hầu như luôn là hai lần đánh nốt thật, không phải một nốt vỡ do nhiễu — merge chỉ dựa vào pitch+gap là không đủ.

Fix: thêm điều kiện duration-ratio — mảnh nốt vỡ do nhiễu thật sự thường **ngắn bất thường** so với nốt chính (vd 40ms dính vào một nốt 1000ms, ratio~0.04), trong khi nốt lặp lại có chủ đích có độ dài **tương đương nhau** (ratio gần 1.0). Test trên file `farran_ez-minimal-piano-underscore-456148.mp3`: 34 → 47 nốt (khôi phục đúng các nốt D5 lặp lại theo nhịp thay vì 1 nốt dài sai); file `jtwayne...`: 42 → 43 nốt.
